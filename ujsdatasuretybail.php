...
$basedir = "c:\Mikes Program Files\wamp\www\suretybail\\";
...
function insertIntoDatabase($name, $oldDocketNum, $docketNum, $FTA, $benchWarrantDate, $judgmentNumber, $judgmentDate, $judgmentAmount, $judgmentAgainstLast, $judgmentAgainstFirst)
{
	...
	$caseID = insertDB("INSERT INTO " . $table . "case (`CPCMS_Docket`, `Legacy_Docket`) values('$docketNum', '$oldDocketNum');");
	
	$defID = insertDB("INSERT INTO " . $table . "dname (`first`, `last`) values('$name[1]', '$name[0]');");
	
	$suretyID = insertDB("INSERT INTO " . $table . "sname (`first`, `last`) values('$judgmentAgainstFirst', '$judgmentAgainstLast');");
	
	insertDB("INSERT INTO " . $table . "judgment (`caseID`, `defID`, `suretyID`, `FTA`, `benchWarrantDate`, `judgmentNumber`, `judgmentDate`, `judgmentAmount`) values('$caseID', '$defID', '$suretyID', '" . dateConvert($FTA) . "', '" . dateConvert($benchWarrantDate) . "', '$judgmentNumber', '" . dateConvert($judgmentDate) . "', '" . str_replace(",","",$judgmentAmount) . "');");
